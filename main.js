document.addEventListener("DOMContentLoaded", function(){
  try{

    onLoadChangeList();
  
  }catch(e){
  
    onLoadChangeList2();
    fixWebsite();
    stateChange();
    miniPanel();
    const header = document.querySelector('header.header');
const divBody = document.querySelector('div.body');
header.removeAttribute('style');
divBody.removeAttribute('style');
  }
  try{

    onLoadChangeList2();
    fixWebsite();
    stateChange();
    miniPanel();
    const header = document.querySelector('header.header');
const divBody = document.querySelector('div.body');
header.removeAttribute('style');
divBody.removeAttribute('style');
  }catch(e){
  
    onLoadChangeList();
  }
});

if( typeof Element.prototype.clearChildren === 'undefined' ) {
Object.defineProperty(Element.prototype, 'clearChildren', {
  configurable: true,
  enumerable: false,
  value: function() {
    while(this.firstChild) this.removeChild(this.lastChild);
  }
});
}

function onLoadChangeList() {
const logoImage = document.querySelector('img.opnav-logo-img');
logoImage.src = browser.runtime.getURL('icons/aternos-logo.png');
}

function onLoadChangeList2() {
const recreateContents = document.createElement('script');
recreateContents.innerHTML = `$(document).ready(function() {
const pageContentBtns = document.querySelector("[style='height:0; width:0; overflow:hidden;']").children;
$('body > span').remove().then($(pageContentBtns).appendTo('div.page-content'));
  $('.body, .header').css({ 'display' : '', 'height' : '' });
    $('#start').each(function() {
      this._ready = true;
    });
})`;
document.head.appendChild(recreateContents);
const enableBtns = document.createElement('script');
enableBtns.innerHTML = `$(document).ready(function() {
$('body > span').remove()
$('.body, .header').css({ 'display' : '', 'height' : '' });
    $('#start').each(function() {
      this._ready = true;
    }).then($('.body, .header').css({ 'display' : '', 'height' : '' }));
})`;
document.head.appendChild(enableBtns);
const debuggingHref = browser.runtime.getURL('aternify-debug/index.html') 
function KeyPress(e) {
  var evtobj = window.event? event : e
  if (evtobj.keyCode == 69 && evtobj.ctrlKey ) window.location.assign(`${browser.runtime.getURL("aternify-debug/index.html")}`);
}  
const pinDownSecretCommand = document.createElement('script');
pinDownSecretCommand.innerHTML = `function pinDownNavServer() {
  const navServer = document.querySelector('div.navigation-server');
  if(!navServer.classList.contains('pinned')) {
    navServer.classList.add('pinned');
  } else {
    navServer.classList.remove('pinned');
  }
}`;
document.head.appendChild(pinDownSecretCommand);
function pinDownNavServer() {
  if(!navServer.classList.contains('pinned')) {
    navServer.classList.add('pinned');
  } else {
    navServer.classList.remove('pinned');
  }
}
document.addEventListener('keydown', KeyPress);
const imgLogo = document.querySelector('a.logo img');
const serverBottomBox = document.querySelector('div.server-info-container')
const skipBtn = document.querySelector('div.btn.btn-white');
const helpBtn = document.querySelector('div.help');
const helpBtn2 = document.querySelector('div.help div');
const profileActs = document.querySelector('div.account-navigation');
const profileActsMobile = document.querySelector('div.userdropdown');
const innerBody = document.querySelector('div.body');
const navigationBar = document.querySelector('nav.navigation');
const sectionContent = document.querySelector('section.content');
const navBarControls = document.querySelector('div.navigation-settings');
const serverBottom = document.querySelector('div.server-bottom-info');
const mobileDarkJs = document.querySelector('a.dropdown-darkmode-toggle');
helpBtn.remove();
profileActs.appendChild(helpBtn2);
helpBtn2.setAttribute("class", "item");
imgLogo.src = browser.runtime.getURL('icons/aternos-logo.png');
//skipBtn.click();
const aternifyBtn = document.createElement('a');
aternifyBtn.setAttribute('class', 'item');
aternifyBtn.setAttribute('title', 'Aternify Ultimate');
aternifyBtn.setAttribute('href', 'https://aternifies.cf/');
aternifyBtn.setAttribute('target', '_blank');
aternifyBtn.innerHTML = '<i class="fas fa-globe-americas"></i><span class="navigation-item-label">    Aternify Ultimate</span>';
navigationBar.insertBefore(aternifyBtn, navBarControls);
const miniPanelBtn = document.createElement('a');
miniPanelBtn.setAttribute('class', 'item');
miniPanelBtn.setAttribute('title', 'Start Mini Panel');
miniPanelBtn.setAttribute('onclick', "window.open('https://aternos.org/server?mode=mini','','postwindow', 'height=1000px', 'width=500px')");
miniPanelBtn.setAttribute('target', '_blank');
miniPanelBtn.innerHTML = '<i class="fas fa-server"></i><span class="navigation-item-label">    Start Mini Panel (Beta)</span>';
const aternifyVer = document.createElement('div');
aternifyVer.setAttribute('class', 'server-info-container');
aternifyVer.innerHTML = `<div class="server-info-box">
<div class="server-info-box-head">
    <div class="server-info-box-title">
        <i class="fa fa-tag"></i> Aternify Version                    </div>
</div>
<div class="server-info-box-body">
    <div class="server-info-box-value">
        <span id="version">Aternify Ultimate 10.1</span>
                                                    </div>
</div>
</div>`;
const divStatus = document.querySelector('header.header');
const oldAlertAPI = document.querySelector('main.main > div.alert-wrapper').remove();
const newAlertAPI = document.createElement('div');
newAlertAPI.setAttribute('class', 'alert-wrapper');
newAlertAPI.innerHTML = `<div class="alert-container"></div>`;
document.body.insertBefore(newAlertAPI, divStatus);
serverBottom.appendChild(aternifyVer);
//navigationBar.insertBefore(miniPanelBtn, aternifyBtn);
const loadingText = document.createElement("div");
loadingText.setAttribute('class', 'loading-text');
loadingText.innerHTML = `<div class="spinner">
<div class="bounce1"></div>
<div class="bounce2"></div>
<div class="bounce3"></div>
</div>`;
loadingText.style = "width: 100%; text-align: center;";
//sectionContent.appendChild(loadingText);
}
function fixWebsite() {
const antiAdBlockEr = document.querySelector('body > span > div > div');
antiAdBlockEr.style = "opacity: 0 !important;"
}
function antiAdBlockerFixedMessage() {
const antiAdBlockerBody = document.querySelector('body > span');
antiAdBlockerBody.clearChildren();
}
function stateChange() {
const antiAdBlock = document.querySelector('body > span');
const innerBody = document.querySelector('div.body');
const loadingText = document.querySelector('div.loading-text');
const header = document.querySelector('header.header');
const divBody = document.querySelector('div.body');
/*window.setTimeout(function() {
  
  loadingText.remove();
}, 3500);*/
//antiAdBlock.remove();
}
function miniPanel() {
  if(window.location.href.indexOf("?mode=mini") > -1) {
    const navigationBarMini = document.querySelector('nav.navigation').remove();
    const headerMini = document.querySelector('div.hamburger').remove();
    const serverBottomMini = document.querySelector('div.server-bottom').remove();
    const serverActivityMini = document.querySelector('div.start-activity-list').remove();
    const accountNavigation = document.querySelector('div.account-navigation').remove();
    const accountNavigation2 = document.querySelector('span.logout').remove();
    const sectionContentMini = document.querySelector('section.content').style = "padding-top: 0px;";
    const userAcc = document.querySelector('div.user').style = "margin-left: auto;";
    const ServerIpMiniTop = document.querySelector('div.server-ip').style = "border-radius: 0px !important; margin-top: 0px !important; margin-bottom: 0px !important; width: auto; margin: 8px -15px; margin-left: -20px !important; margin-right: -30px !important;";
    const ServerIpMiniBottom = document.querySelector('div.server-status').style = "margin-top: 0px !important; margin-bottom: 0px; width: auto; margin: 8px -15px;  margin-left: -20px !important; margin-right: -30px !important; border-radius: 0px !important;";
    const ServerIpMiniBottom2 = document.querySelector('div.status').style = "border-radius: 0px !important;";
    const ServerIpMiniTopBtn = document.querySelector('div.server-ip > a.btn').remove();
    const queueMsg = document.querySelector('div.queue-message').style = "border-radius: 0px !important; margin: 0px !important; margin-left: -20px !important; margin-right: -30px !important;";
    document.querySelector('title').innerHTML = "Aternos Mini Panel (Beta)";
  } else {
    return;
  }
}
